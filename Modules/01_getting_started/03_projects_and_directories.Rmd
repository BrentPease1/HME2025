...
```{r, eval = FALSE}
rm(list = ls())
setwd("C:\Users\bpease\path\specific\to\my\machine")
...
5) Start googling around because you are stuck on some code. Paste in Stack Overflow suggestions with generic object names.
...
In RStudio, you can enforce this workflow by going to `RStudio -> Tools -> Global Options` and then in the `General` tab setting `Save workspace to .Rdata on exit` to `Never`. Highly recommended.  
...
|─── results/
      |─── cool_result.rds
      |─── extra_cool_result.rds
|─── figures/
      |─── fig1.png
      |─── fig2.png
```

* `data/` contains my, you guessed it, data files (CSVs, shapefiles, etc). These data files are read into scripts for processing, analyzing, etc. 

* `scripts/` contains R scripts used to analyze data, make figures, whatever. These could be put in the subdirectory related to the function of the script if that makes more sense to you, or name the file such that it tells you its function. Any grouping/ordering is fine, as long as it makes sense to you and you know how to find it when you need it.

* `results/` contains output from any scripts that may be used in other scripts for further analyses.

* `figures/` contains figure files.    

I have seen more and less complicated folder structures, but the above has worked well for me. If there are more folders, it usually is for intermediate steps.
...
If you prefer, you can have as many layers of subdirectories as you want. The important thing is that as long as you move the entire project directory, these relative paths will still work. One note is that, in rare cases, the path with the final file name can get too long and throw an error (that will likely result in a long period of time before you figure out the issue).  
...
Just as important, you don't save these new data objects over the raw data or even in the same directory. Those files go in `results/` folder or, perhaps an `output/` folder to hold intermediate objects. This means you can only read from the `data/` and you cannot write to it. R will, of course, allow you to write to `data/`, this is just a suggestion to help you maintain a firewall that will keep you raw data in it's raw form. 
...
[Swirl](https://swirlstats.com/) is a way to learn R *within* R. Though based in the Tidyverse, it is a package that contains interactive R learning exercises. You will find exercises for learning the basics in R, data visualization, etc..